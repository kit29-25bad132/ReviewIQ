...
class EvaluationService:
    ...
    def metrics(self):
        ...
        total = len(rows)
        return EvaluationMetrics(evaluated_reviews=total, rating_accuracy=round(correct / total, 4), rating_mae=round(absolute_error / total, 4), sentiment_accuracy=round(sentiment_correct / total, 4), confusion_matrix=matrix, methodology="Actual sentiment is derived from ratings: 1–2 negative, 3 neutral, 4–5 positive.")
    ...
    def run(self, limit: int | None = None, reanalyze: bool = False):
        ...
        with self._lock:
            ...
            for record in records:
                ...
                cache[record.id] = {"actual_rating": record.actual_rating, "predicted_rating": ai.rating, "actual_sentiment": record.actual_sentiment, "predicted_sentiment": ai.sentiment, "pros": ai.pros, "cons": ai.cons, "summary": ai.summary, "timestamp": datetime.now(timezone.utc).isoformat()}
            self._save(cache)
        return self.metrics()
